...
NSString * const LGPrefsDomain = @"com.ash.liquidassglobalswitch";
CFStringRef const LGPrefsChangedNotification = CFSTR("com.ash.liquidassglobalswitch/preferences.changed");
CFStringRef const LGPrefsRespringNotification = CFSTR("com.ash.liquidassglobalswitch/Respring");
const char * const LGPrefsChangedNotificationCString = "com.ash.liquidassglobalswitch/preferences.changed";
const char * const LGPrefsRespringNotificationCString = "com.ash.liquidassglobalswitch/Respring";
...
static NSString * const LGPrefsDidReloadInProcessNotification = @"com.ash.liquidassglobalswitch.InProcessReload";
...
static NSString *LGLogFilePath(void) {
    ...
    dispatch_once(&onceToken, ^{
        sPath = @"/tmp/LiquidAssGlobalSwitch.log";
    });
    return sPath;
}
...
static void LGAppendLogLine(NSString *line) {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sLGLogQueue = dispatch_queue_create("com.ash.liquidassglobalswitch.logfile", DISPATCH_QUEUE_SERIAL);
    ...
    dispatch_async(sLGLogQueue, ^{
        ...
        if (@available(iOS 13.0, *)) {
            ...
            @try {
                ...
                handleError = [NSError errorWithDomain:@"com.ash.liquidassglobalswitch.logfile"
                                                  code:1
...
static void LGScheduleDynamicDefaultFlush(void) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sLGPrefsWriteQueue = dispatch_queue_create("com.ash.liquidassglobalswitch.prefswrite", DISPATCH_QUEUE_SERIAL);
    });
...
BOOL LG_globalEnabled(void) {
    return LG_prefBool(@"enabled", YES);
}
